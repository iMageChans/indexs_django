...
async function MarketMaker(ctx: ProcessorContext<Store>){
    for await (const block of ctx.blocks) {
        for await (const event of block.events) {
            ...
            if (decodedEvent.__kind === "LiquidityAdded") {
                data = {
            ...
            } else if (decodedEvent.__kind === "LiquidityRemoved") {
                data = {
                    block_hash: block.header.hash,
                    block_number: block.header.height,
                    d9: decodedEvent.d9,
                ...
                await TokenTransfers(data)
            } else if (decodedEvent.__kind === "D9ToUSDTConversion") {
                data = {
                    block_hash: block.header.hash,
                    block_number: block.header.height,
                    d9: decodedEvent.d9,
            ...
            } else if (decodedEvent.__kind === "USDTToD9Conversion") {
                data = {
                    block_hash: block.header.hash,
                    block_number: block.header.height,
                    d9: decodedEvent.d9,
